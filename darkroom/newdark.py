...
# Function to open the Black Eyes image window using tkinter
...
# Function to open the black eyes image window in a separate thread
...
# Function to open the "NOBDODY LEAVES THE DARKROOM" message window when trying to exit using tkinter
...
def process_command(command):
    global running, match_lit, first_match_lit, matches_left, commands_since_last_match, inventory, current_scene_items, awaiting_exit_confirmation, current_image
    
    # Trying to exit the darkroom commands
    if awaiting_exit_confirmation:
        if command.lower() in ["y", "yes"]:
            add_output("That's nice.", "exit_yes")
        else:
...
#-----------------------------------------------------------------------------------------------------------------------#
def main():
    global command, running, current_image, output_lines, matches_left, match_lit, first_match_lit, commands_since_last_match, inventory, current_scene_items, awaiting_exit_confirmation
    display_intro()
    output_lines.clear()
    while running:
        screen.fill(BLACK)
        ...
        text_surface = FONT.render(f"> {command}", True, WHITE)
        screen.blit(text_surface, (10, SCREEN_HEIGHT - 30))

        # Draw the current image on the right side if it is set
        screen.blit(current_image, (TEXT_AREA_WIDTH, 0))
